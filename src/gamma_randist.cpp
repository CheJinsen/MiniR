...
double Gamma::pdf(double x, double shape, double scale, bool give_log)
{
	...
	double pr = 0.0;
	if (shape < 1) {
		pr = Base::poissonPdfRaw(shape, x / scale, give_log);
		return give_log
	...
	pr = Base::poissonPdfRaw(shape - 1, x / scale, give_log);
...
double Gamma::poissonPdfWrap(double x_plus_1, double lambda, bool give_log)
{
	...
	if (x_plus_1 > 1)
		return Base::poissonPdfRaw(x_plus_1 - 1, lambda, give_log);
	...
	if (lambda > fabs(x_plus_1 - 1) * M_cutoff) {
		...
		double d = Base::poissonPdfRaw(x_plus_1, lambda, give_log);
...
double Gamma::cdfSmallx(double x, double alph, bool lower_tail, bool log_p)
{
	...
	if (lower_tail) {
		...
		double f2 = 0.0;
		if (alph > 1) {
			f2 = Base::poissonPdfRaw(alph, x, log_p);
