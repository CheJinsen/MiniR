...
double dnbinom(double x, double size, double prob, bool give_log)
{
    ...
    double ans = dbinom_raw(size, x + size, prob, 1 - prob, give_log);
    double p = ((double)size) / (size + x);
    return give_log ? log(p) + ans : p * ans;
}

double dnbinom_mu(double x, double size, double mu, bool give_log)
{
